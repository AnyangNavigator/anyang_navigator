...
| `facilities_bus_stop.csv` | 안양시 버스정류장 773건 | 국토교통부 | data.go.kr 전국 버스정류장 위치정보 (전국 파일, `도시명='경기도 안양시'` 필터) | 2025-10-31 |
| `facilities_school.csv` | 안양시 초·중·고 86건 (초41·중24·고21) | 교육부 | data.go.kr 전국초중등학교위치표준데이터 (안양시분, 소재지 주소로 필터) | 2026-03-20 |

> `facilities_bus_stop.csv`는 원본이 전국 22만여 건 규모라 안양시분만 걸러 커밋했습니다(원본 컬럼 그대로 보존, 한글 헤더). 좌표→행정동 매핑 실패 3건(0.4%, 안양-의왕 경계 인근 정류장)은 매핑 실패율 가드(10%) 이내입니다. 지하철역은 이번 소싱에 포함하지 않았습니다 — 국가철도공단 역사정보(별도 데이터셋) 확보 시 후속.
>
