...
행정동 경계 GeoJSON(`data/anyang_dong_boundaries.geojson`)은 [국가데이터처_SGIS 행정구역 통계 및 경계](https://www.data.go.kr/data/15129688/fileData.do) (data.go.kr, 기준일 2025-06-30) 공식 공공데이터에서 `scripts/extract_anyang_geojson.py`로 추출한 것입니다. 원본 좌표계(EPSG:5179)를 WGS84로 변환하고, 안양시의 2025-04-30 동 이름 개칭(박달1동→박달동, 박달2동→호현동)을 반영합니다. (이전에는 개인 저장소 vuski/admdongkor를 썼으나 이슈 #30에 따라 공식 출처로 교체했습니다.) **전체 데이터 출처는 [`data/SOURCES.md`](data/SOURCES.md)** 참조.
...
2. 서비스 환경변수 설정 (Render 대시보드):
   - `OPENAI_API_KEY` — 선택. 없으면 규칙 기반 리포트로 폴백(앱은 정상 작동). 무료로 쓰려면 위 표대로 `OPENAI_BASE_URL`·`OPENAI_MODEL`도 함께 설정
...
잠든 상태의 첫 요청은 실제로 **60초를 넘겨 타임아웃**하는 것을 확인했습니다. 심사위원·공개검증 방문자가 링크를 처음 누를 때 그대로 겪는 문제라, `.github/workflows/keep-alive.yml`이 **09:00~22:59 KST에 10분 간격으로 `/healthz`를 핑**해 인스턴스를 깨워둡니다.

- 24시간이 아닌 이유: Render 무료 플랜은 **월 750 인스턴스 시간** 한도라, 24시간 상시 가동(720~744h)은 한도에 거의 붙어 초과 시 서비스가 멈춥니다. 현재 설정은 월 420~434시간으로 여유가 있습니다.
- 시간대를 넓히려면 워크플로의 `cron: "*/10 0-13 * * *"`에서 시간 범위(`0-13`, UTC 기준)를 조정하세요.
- 심사 직전 즉시 깨우려면 **Actions 탭 → Keep alive → Run workflow**.
